    ...
    <? if ($USER->IsAuthorized() && ($arItem["MIN_PRICE"])) { 
		...
		<?php if(!$arParams["HIDE_CONTROLS"]): ?>
			<? if ($arItem['CATALOG_QUANTITY'] > 0 ) { ?>
				...
				<div class="catalog-item-price"><? if (!empty($arItem['MIN_PRICE']['DISCOUNT_DIFF']) && $arItem["MIN_PRICE"]["SIMPLE_DISCOUNT"] == "Y") { ?><span class="catalog-item-price-old"><?=$arItem['MIN_PRICE']['PRINT_VALUE']?></span><? } ?><span class="catalog-item-price-current"><?=$arItem['MIN_PRICE']['PRINT_DISCOUNT_VALUE']?></span></div>
