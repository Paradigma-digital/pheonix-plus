...
function updateBasketTable(basketItemId, res)
{
			...
			if (res.BASKET_DATA.GRID.ROWS.hasOwnProperty(id))
			{
				...
				if (BX('sum_' + id))
					BX('sum_' + id).innerHTML = item.SUM;
                                    console.log(item);
				if (BX('tax_' + id))
					BX('tax_' + id).innerHTML = Math.round((item.VAT_VALUE*item.QUANTITY*100))/100 + '&nbsp;₽';
				...
				if (BX('custom_tax_' + id)) {
					BX('custom_tax_' + id).innerHTML = (item.SUM_VALUE*item.VAT_RATE).toFixed(2) + '&nbsp;₽';
				}
				if (BX('sum_with_tax_' + id)) {
					BX('sum_with_tax_' + id).innerHTML = (parseFloat((item.SUM_VALUE*item.VAT_RATE).toFixed(2)) + item.SUM_VALUE).toFixed(2) + '&nbsp;₽';
				}				
	...
	if (!!res.BASKET_DATA)
	{
		...
		if (BX('allSum_ItemsCount')) {
			BX('allSum_ItemsCount').innerHTML = res['BASKET_DATA']["ORDERABLE_BASKET_ITEMS_COUNT"];
		}
			
		
		
		
		if (BX('allSum_FORMATED2')) {
			BX('allSum_FORMATED2').innerHTML = res['BASKET_DATA']['allSum_FORMATED'].replace(/\s/g, '&nbsp;');
		}
...
function updateQuantity(controlId, basketId, ratio, bUseFloatQuantity, maxAvailable, refresh = false)
{
	...
	//console.log(newVal, maxAvailable);
