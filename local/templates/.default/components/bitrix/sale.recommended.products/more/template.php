    ...
    <div class="page-inner page-inner--w1">
            <div class="catalog">
              <div class="catalog-header">
                <div class="catalog-title">С этим товаром покупают</div>
              ...
              <div <?php if(count($arResult["ITEMS"]) > 4): ?>data-slick="{&quot;slidesToShow&quot;: 4, &quot;slidesToScroll&quot;: 4}"<?php endif; ?> class="catalog-items">
		  <? foreach ($arResult['ITEMS'] as $arItem) { //deb($arItem['MIN_PRICE'], false)?>
                <div class="catalog-item catalog-item--w1">
                  <div class="catalog-item-inner"><a href="<?=customDetailUrl($arItem['DETAIL_PAGE_URL'])?>" class="catalog-item-link animation-link">
				...
				<?php endif; ?> 
					...
					<div class="catalog-item-buy-holder">
                                            <div class="catalog-quantity-line"
                                                 data-value="<?php echo $arItem["PROPERTIES"]["PROTSENT_DOSTUPNOSTI"]["VALUE"]; ?>"></div>
                                            <?
                                            if ($arItem["SHOW_PRE_ORDER"] == "Y") {
                                                ?>
                                                <input type="text"
                                                       value="<?= $krat ?>"
                                                       data-pack="<?= $krat ?>"
                                        ...
                                        <div class="catalog-item-buy-holder" style="top:5px">
                                            <input type="text"
                                                   value="<?= $krat ?>"
                                                   data-pack="<?= $krat ?>"
                                                   data-id="<?php echo $arItem['ID']; ?>"
                                            ...
                                            <a data-q="<?= $inCartArray['UF_USER_CART'][$arItem['ID']] ?>"
                                               href="javascript:void(0)"
                                               class="catalog-item-buy btn btn--gray product-buy">

                                                <?php if (!empty($inCartArray['UF_USER_CART'][$arItem['ID']])): ?>
                                                    В корзине <?php echo $inCartArray['UF_USER_CART'][$arItem['ID']] ?>
                                                <?php else: ?>
                                                    Купить
                                                <?php endif; ?>
                                            </a>
