...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function(parameters)
		{
			...
			this.preOrder = parameters.preOrder;
					...
					onsuccess: BX.delegate(function(result) {
						...
						switch (action)
						{
							...
							case 'showAuthForm':
								...
								this.refreshOrder(result);
								if(!result.order.ERROR.AUTH) {
									if(result.order.AUTH.NEW_EMAIL && window.ym && window.ym != undefined) {
										ym(48640211, "reachGoal", "event_user_add", {
											login: result.order.AUTH.NEW_EMAIL
										});
									...
									if(!result.order.AUTH.NEW_EMAIL) {
		...
		refreshOrder: function(result)
		{
			...
			console.log(result);
			if(!result.error && $.isEmptyObject(result.order.ERROR) && $.isEmptyObject(result.order.DELIVERY)) {
				result.error = "Доставка по указанному адресу невозможна";
				this.showError(this.mainErrorsNode, result.error);
				this.animateScrollTo(this.mainErrorsNode, 800, 20);
				$("[data-save-button]").hide();
			} else {
				$("[data-save-button]").show();
			}
		...
		getBlockFooter: function(node)
		{
				...
				buttons.push(
					BX.create('A', {
						props: {
							...
							className: 'pull-left button button--gray'
						},
			...
			if (!isLastNode)
			{
				buttons.push(
					BX.create('A', {
						props: {href: 'javascript:void(0)', className: 'pull-right button button--blue'},
		...
		editOrder: function()
		{
			...
			$("input[data-cmask]").mask("+7 (000) 000 00 00", {
				...
				children: [
					...
					BX.create('A', {
						props: {
							...
							href: '/forgot/?back_url=' + encodeURIComponent(document.location.href)
						},
		...
		createAuthFormInputContainer: function(labelText, inputNode, required)
		{
			...
			if (required)
				labelHtml += ' <span class="form-required">*</span>';

			return BX.create('DIV', {
				props: {className: 'form-row'},
				children: [
					BX.create('DIV', {props: {className: 'form-label form-label--block'}, html: labelHtml}),
					BX.create('DIV', {props: {className: 'form-item-holder form-item-holder--block'},  children: [inputNode]})
				]
			});
		},
			...
			else
			{
						...
						children: [
							...
							BX.create('INPUT', {
								props: {
									...
									type: 'hidden',
									size: 30,
									value: this.result.AUTH.NEW_LOGIN || 'test'
								},
								events: {
									keypress: BX.proxy(this.checkKeyPress, this)
								}
				...
				if (this.result.AUTH.captcha_registration == 'Y')
				{
					authFormNodes.push(BX.create('DIV', {
						...
						children: [
							BX.create('DIV', {
								props: {className: 'form-label form-label--block'},
								children: [
							...
							BX.create('DIV', {
								props: {className: 'form-item-holder form-item-holder--block'},
								children: [
									BX.create('INPUT', {
										attrs: {'data-send': true},
										props: {
				...
				authFormNodes.push(
					BX.create('DIV', {
						...
						children: [
							...
							BX.create('A', {
								props: {className: 'link link--black', href: ''},
			...
			if (this.result.AUTH.new_user_registration === 'Y')
			{
				nodes.push(BX.create('DIV', {
					...
					children: [
						...
						BX.create('A', {
							props: {className: 'btn btn--blue', href: "/register/"},
		...
		createBasketItemContent: function(data)
		{
			...
			if (this.params.HIDE_DETAIL_PAGE_URL !== 'Y' && data.DETAIL_PAGE_URL && data.DETAIL_PAGE_URL.length)
			{
				titleHtml = '<a href="' + data.DETAIL_PAGE_URL + '" class="link link--black">' + titleHtml + '</a>';
			}
		...
		getPersonTypeControl: function(node)
		{
			...
			{
				input = BX.create('DIV', {
					props: {className: 'form-group form-group--hidden'},
					children: [
		...
		editPaySystemItems: function(paySystemNode)
		{
			...
			var paySystemItemsContainer = BX.create('DIV', {props: {className: 'col-sm-6 bx-soa-pp-item-container'}}),
				paySystemItemNode, i;
		...
		editPaySystemInfo: function(paySystemNode)
		{
			...
			var paySystemInfoContainer = BX.create('DIV', {
					props: {
						className: (this.result.PAY_SYSTEM.length == 0 ? 'col-sm-12' : 'col-sm-6') + ' bx-soa-pp-desc-container'
					}
				}),
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			{
				BX.addClass(propsItemNode, "form-row form-row--lm form-group bx-soa-customer-field");

				if(property.getName() == "Пункт самовывоза") {
					BX.addClass(propsItemNode, "form-row--hidden");
				}

				textHtml += BX.util.htmlspecialchars(property.getName());

				if (property.isRequired()) {
					textHtml += '<span class="form-required">*</span> ';
				}
				...
				label = BX.create('LABEL', {
					...
					props: {className: 'bx-soa-custom-label form-label form-label--block'},
					html: textHtml
				});
		...
		alterProperty: function(settings, propContainer)
		{
			...
			if (textNode)
			{
				...
				if(settings.CODE == "PHONE") {
					textNode.setAttribute('data-cmask', 'phone');
				}
			}
			...
			for (i = 0; i < inputs.length; i++)
			{
				...
				BX.addClass(inputs[i], 'form-item form-item--text');
		...
		editPropsComment: function(propsNode)
		{
			...
			div = BX.create('DIV', {
				props: {className: 'form-row form-row--btn form-group bx-soa-customer-field'},
