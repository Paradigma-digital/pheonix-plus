...
class CBitrixBasketComponent extends CBitrixComponent
{
	...
	protected function initialLoadAction()
	{
		...
		// На первой загрузке нужно вернуть только шаблон
	...
	protected function recalculateAction()
	{
		...
		$result = $this->recalculateBasket($this->request->toArray()); // Быстрый метод, 0.02 сек.
		...
		$result['BASKET_DATA'] = $this->getBasketItems(); // Долгий метод, его нужно оптимизировать
	...
	protected function processBasketItem(Sale\BasketItem $item)
	{
		...
		$basketItem['HASH'] = $this->getBasketItemHash($basketItem);
			...
			// Кастомизация. Получение свойств "артикул" и "кол-во в упаковке"
			$arArticleProp = CGCHelper::getIBlockProperty([
				"IBLOCK_ID" => "9",
			...
			$arTraitsProps = CGCHelper::getIBlockProperty([
				"IBLOCK_ID" => "9",
				"ID" => $basketItem["PRODUCT_ID"],
				"FILTER" => [
					"CODE" => "CML2_TRAITS"
			...
			$basketItem["PRODUCT_PROPS"] = [
				"ARTICLE" => $arArticleProp[0],
				//"TRAITS" => $arTraitsProps,
			];
			foreach($arTraitsProps as $arTraitsPropItem) {
				if($arTraitsPropItem["DESCRIPTION"] == "КратностьОтгрузки") {
					$basketItem["PRODUCT_PROPS"]["PACK"] = $arTraitsPropItem;
				}
			}


			$basketItem["IN_FAVORITES"] = СGCBookmark::isProductInFavorites($basketItem["PRODUCT_ID"]);
	...
	protected function fillItemsWithProperties()
	{
		...
		foreach ($this->basketItems as &$item)
		{
			...
			{
				$arTmpImg = CGCHelper::resizeImage($item["PREVIEW_PICTURE"], 300, 300);
				$item['PREVIEW_PICTURE_SRC'] = $arTmpImg["SRC"];
