...
class CSaleExportAhead extends CSaleExport {
...
<<?=CSaleExport::getTagName("SALE_EXPORT_COM_INFORMATION")?> <?=self::getCmrXmlRootNameParams()?>><?
		...
		while($arOrder = $dbOrderList->Fetch())
		{
			...
			AddMessage2Log($propertyCollection, "propertyCollection"); // sdemon72
			$xmlResult["OrderTax"] = self::getXMLOrderTax($arOrderTax, $propertyCollection->getItemByOrderPropertyId(16)->getValue());
			//deb($xmlResult["OrderTax"]); die;
	...
	static function getXmlOrderTax($arOrderTaxAll, $customType) {
		$strResult = "";
		if(is_array($arOrderTaxAll) && count($arOrderTaxAll)>0)
		{
			...
			foreach ($arOrderTaxAll as $arOrderTax )
			{
				...
				$strResult .= "<".CSaleExport::getTagName("SALE_EXPORT_TAX").">".
					...
					"<".CSaleExport::getTagName("SALE_EXPORT_IN_PRICE").">".(($customType == "OVER") ? "false" : "true")."</".CSaleExport::getTagName("SALE_EXPORT_IN_PRICE").">".
					"<".CSaleExport::getTagName("SALE_EXPORT_AMOUNT").">".($customType == "NO" ? "0" : $arOrderTax["VALUE_MONEY"])."</".CSaleExport::getTagName("SALE_EXPORT_AMOUNT").">".
...
class CSaleOrderLoaderAhead extends CSaleOrderLoader {
		...
		if($importer instanceof Sale\Exchange\ImportOneCBase)
		{
			...
			if($r->isSuccess())
			{
				...
				if($r->isSuccess())
				{
					...
					$arXmlOrders = $value;
					if(1) {
						foreach($arXmlOrders["Контейнер"]["#"]["Документ"] as $arXmlOrder) {
							//deb($arXmlOrder);
							$arOrder = [
							...
							foreach($arXmlOrder["#"]["СвойстваЗаказа"][0]["#"]["СвойствоЗаказа"] as $arXmlOrderProp) {
								//deb($arXmlOrderProp);
								$arOrder["PROPS"][] = [
									"NAME" => $arXmlOrderProp["#"]["Наименование"][0]["#"],
									"VALUE" => $arXmlOrderProp["#"]["Значение"][0]["#"]
								];
							}
							
							
							// Обновление корзины
							foreach($arXmlOrder["#"]["Товары"][0]["#"]["Товар"] as $arXmlOrderProduct) {
								$productID = $arXmlOrderProduct["#"]["Ид"][0]["#"];
								$productProps = [];
								//deb($arXmlOrderProduct); die;
								foreach($arXmlOrderProduct["#"]["ДополнительныеДанные"][0]["#"]["ЗначениеРеквизита"] as $arXmlOrderProductProp) {
									$productProps[] = [
										"NAME" => $arXmlOrderProductProp["#"]["Наименование"][0]["#"],
										"VALUE" => $arXmlOrderProductProp["#"]["Значение"][0]["#"],
									];
								}
								$arOrder["BASKET"][$productID] = [
									"PROPS" => $productProps,
								];
							}
							...
							$dbBxOrder = CSaleOrder::GetList([
								"DATE_INSERT" => "ASC"
							], [
								"ID_1C" => $arOrder["ID_1C"],
							]);
							$arBxOrder = $dbBxOrder->Fetch();
							...
							//deb($arOrder["BASKET"]); die;
							
							$basket = Sale\Order::load($arBxOrder["ID"])->getBasket();
							$basketItems = $basket->getBasketItems();
							foreach ($basketItems as $basketItem) {
								if($arOrder["BASKET"][$basketItem->getField("PRODUCT_XML_ID")]["PROPS"]) {
									$basketPropertyCollection = $basketItem->getPropertyCollection();
									
									$basketPropertyCollection->setProperty($arOrder["BASKET"][$basketItem->getField("PRODUCT_XML_ID")]["PROPS"]);
								}
							}
							$basket->save();
							//deb($basketItems); die;
							...
							while($arBasketItem = $dbBasketItems->Fetch()) {
								if($arOrder["BASKET"][$arBasketItem["PRODUCT_XML_ID"]]) {
									CSaleBasket::Update($arBasketItem["ID"], [
										"PROPS" => $arOrder["BASKET"][$arBasketItem["PRODUCT_XML_ID"]]["PROPS"]
									]);
								}
							}*/
							
							//$time_end = microtime(true);
							//echo '<b>Total Execution Time:</b> '.(($time_end - $time_start)/60).' Mins<br />';
							
							
							
							foreach($arOrder["PROPS"] as $arOrderProp) {
								...
								$dbBxOrderProp = CSaleOrderPropsValue::GetList(
									[
										"ID" => "ASC"
									],
									[
								...
								$arBxOrderProp = $dbBxOrderProp->Fetch();
	...
	function nodeHandler2(CDataXML $dataXml, CXMLFileStream $fileStream)
	{
...
class CSaleOrderLoaderAheadDeb
{
