<?php
	...
	if($_GET["mode"] == "checkauth" && $USER->IsAuthorized()) {
		...
		if($_GET["mode"]=="init") {
			...
			$_SESSION["reference_name"] = "BDocHeading";
			
			// Формирование массива для обработки
			$arItems = GCDocs::exportItems($_SESSION["reference_name"]);
		...
		// Отдаем файл с юзерами
			...
			if($_SESSION["get_reference_EXPORTED_IDS"]) {
				...
				foreach($_SESSION["get_reference_EXPORTED_IDS"] as $exportedItemID) {
					CGCHL::update(CGCHL::getHLByName("BDocHeading")["ID"], $exportedItemID, [
						"UF_EXPORTED" => 1
					]);
				}
			}
			...
			if($_SESSION["get_reference_FOR_EXPORT"]) {
				if($fp = fopen("php://output", "ab")) {
					...
					if($_SESSION["get_reference_LAST_EXPORTED_INDEX"] == -1) {
						...
						fwrite($fp, GCDocs::makeXMLFromTemplate(false, true, false, [
							"reference_name" => $_SESSION["reference_name"],
						]));
						fwrite($fp, GCDocs::makeXMLFromTemplate(false, false, true));
						$i = -1;
						...
						if(count($_SESSION["get_reference_FOR_EXPORT"]) == 1) {
							...
							fwrite($fp, GCDocs::makeXMLFromTemplate([$arExportItem], true, false, [
							"reference_name" => $_SESSION["reference_name"],
						]));
							fwrite($fp, GCDocs::makeXMLFromTemplate(false, false, true));
						...
						// Случай нескольких
							...
							for($i = $_SESSION["get_reference_LAST_EXPORTED_INDEX"]; $i < count($_SESSION["get_reference_FOR_EXPORT"]); $i++) {
								...
								if($firstItem) {
									$exportItemXml = GCDocs::makeXMLFromTemplate([$arExportItem], true, false, [
										"reference_name" => $_SESSION["reference_name"],
									]);
									$firstItem = false;
								} else {
									$exportItemXml = GCDocs::makeXMLFromTemplate([$arExportItem], false, false);
									if((ob_get_length() + strlen($exportItemXml)) >= 204800 * 0.9) {
										$exportItemXml = GCDocs::makeXMLFromTemplate(false, false, true);
										$lastItem = true;
									}
								}
